...
def denoise_block(
    ...
    *,
    ...
    if context_frames is not None:
        times_zero = torch.zeros(
            [B, context_frames.shape[1]],
            device=device,
            dtype=torch.int64,
        )
    ...
    for index, current_timestep in enumerate(denoising_steps):
        ...
        timestep = (
            torch.ones([B, F], device=device, dtype=torch.int64) * current_timestep
        )
...
class CausalInferencePipeline(torch.nn.Module):
    def __init__(self, args, device, generator=None, text_encoder=None, vae=None):
        ...
        self.generator = WanDiffusionWrapper(
            **getattr(args, "generator", {}), is_causal=True
        )
        ...
        text_encoder_model_folder = (
            getattr(args, "text_encoder_model_folder", None) or wan_model_folder
        )
        vae_model_folder = getattr(args, "vae_model_folder", None) or wan_model_folder
        self.text_encoder = (
            WanTextEncoder(model_folder=text_encoder_model_folder)
            if text_encoder is None
            else text_encoder
        )
        print(
            f"Time taken to initialize text encoder: {time.time() - time_start} seconds"
        )

        time_start = time.time()
        self.vae = WanVAEWrapper(model_folder=vae_model_folder) if vae is None else vae
        ...
        self._model_num_heads = self.generator.model.config.num_heads
        self._model_dim = self.generator.model.config.dim
    ...
    def inference(
        ...
        decode: bool = True,
        use_kv_sink: bool = False,
        kv_sink_window: int = 3,
    ) -> torch.Tensor:
        """
        ...
        assert num_frames % self.num_frame_per_block == 0, (
            f"num_frames {num_frames} is not a multiple of num_frame_per_block {self.num_frame_per_block}"
        )
        ...
        if use_kv_sink:
            required_kv = 1560 * nfpb * (1 + kv_sink_window)
        else:
            required_kv = 1560 * nfpb * 2
        ...
        if self.kv_cache1 is None:
            self._initialize_kv_cache(
                ...
                dtype=noise.dtype,
                device=noise.device,
                kv_cache_size=required_kv,
            )
        elif self.kv_cache1[0]["k"].shape[1] < required_kv:
            self.kv_cache1 = None
            self._initialize_kv_cache(
                batch_size=batch_size,
                dtype=noise.dtype,
        ...
        all_num_frames = [nfpb] * num_blocks

        full_timeline_ref = ref_latent.shape[1] == num_frames
        sink_kv = None
        window_outputs = collections.deque(maxlen=kv_sink_window)
        ...
        for num_block_frame in all_num_frames:
            ...
            if full_timeline_ref:
                ref_block = ref_latent[:, start_index : start_index + num_block_frame]
            elif start_index == 0:
                ref_block = ref_latent[:, :num_block_frame]
            else:
                ref_block = torch.zeros(
                    batch_size,
                    num_block_frame,
                    num_channels,
                    height,
            ...
            ref_block = ref_block.to(device=noise.device, dtype=noise.dtype)

            # Clear KV cache each block
            for bc in self.kv_cache1:
                bc["k"].detach_().zero_()
                bc["v"].detach_().zero_()

            if start_index == 0:
                # Block 0: context = ref_padded
                zero_latents = torch.zeros_like(ref_block)
                context_frames = torch.cat(
                    [ref_block, zero_latents[:, :, :4], zero_latents], dim=2
                )
                kv_size = sink_len
                denoising_freqs = nfpb
                ...
                denoised_pred, _ = denoise_block(
                    ...
                    denoising_freqs_offset=denoising_freqs,
                )

                if use_kv_sink:
                    sink_kv = [
                        {
                            "k": bc["k"][:, :sink_len].clone(),
                            "v": bc["v"][:, :sink_len].clone(),
                        }
                        for bc in self.kv_cache1
                    ]

            elif use_kv_sink and sink_kv is not None:
                # Restore block-0 sink KV
                for i, bc in enumerate(self.kv_cache1):
                    bc["k"][:, :sink_len] = sink_kv[i]["k"]
                    bc["v"][:, :sink_len] = sink_kv[i]["v"]

                # Build sliding window from recent block outputs
                window_frames = torch.cat(list(window_outputs), dim=1)
                z_w = torch.zeros_like(window_frames)
                window_context = torch.cat([window_frames, z_w[:, :, :4], z_w], dim=2)
                num_win_frames = window_frames.shape[1]

                kv_size = sink_len + 1560 * num_win_frames
                denoising_freqs = nfpb + num_win_frames
                ...
                denoised_pred, _ = denoise_block(
                    ...
                    conditional_dict,
                    self.kv_cache1,
                    context_frames=window_context,
                    context_no_grad=True,
                    context_freqs_offset=nfpb,
                    context_kv_size_0=sink_len,
                    render_block=render_block,
                    denoising_kv_size=kv_size,
                    denoising_steps=self.denoising_step_list,
                    denoising_freqs_offset=denoising_freqs,
                ...
                # Original: [ref_padded, prev_padded]
                zero_latents = torch.zeros_like(ref_block)
                ref_block = torch.cat(
                    [ref_block, zero_latents[:, :, :4], zero_latents], dim=2
                )
            ...
            if use_kv_sink:
                window_outputs.append(last_pred)
    ...
    def _initialize_kv_cache(
        self, batch_size, dtype, device, kv_cache_size=None, use_prope=True
    ):
        """
        ...
        If kv_cache_size changed, reallocate.

        When use_prope=True, also allocates k_prope/v_prope for ProPE dual-path.
        """
        ...
        if (
            ...
            and (not use_prope or "k_prope" in self.kv_cache1[0])
        ):
            for block_cache in self.kv_cache1:
                block_cache["k"].detach_().zero_()
                block_cache["v"].detach_().zero_()
                if use_prope and "k_prope" in block_cache:
                    block_cache["k_prope"].detach_().zero_()
                    block_cache["v_prope"].detach_().zero_()
            return

        num_heads = self._model_num_heads
        dim = self._model_dim

        print(f"Initializing kv cache with size: {kv_cache_size} (prope={use_prope})")
        self.kv_cache1 = []
        for _ in range(self.num_transformer_blocks):
            block_cache = {
                "k": torch.zeros(
                    [batch_size, kv_cache_size, num_heads, dim // num_heads],
                    dtype=dtype,
                    device=device,
                ),
                "v": torch.zeros(
            ...
            if use_prope:
                block_cache["k_prope"] = torch.zeros(
                    [batch_size, kv_cache_size, num_heads, dim // num_heads],
                    dtype=dtype,
                    device=device,
                )
                block_cache["v_prope"] = torch.zeros(
                    [batch_size, kv_cache_size, num_heads, dim // num_heads],
                    dtype=dtype,
                    device=device,
                )
            self.kv_cache1.append(block_cache)
